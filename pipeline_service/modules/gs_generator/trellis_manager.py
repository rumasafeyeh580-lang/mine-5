...
class TrellisService:
    ...
    def generate(
        ...
        logger.info(
            f"Generating Trellis {trellis_request.seed=} "
            f"and image size {trellis_request.images[0].size}"
        )
        ...
        try:
            ...
            logger.success(
                f"Trellis finished generation in {generation_time:.2f}s "
                f"with {num_voxels} occupied voxels."
            )
            return result
        finally:
            if buffer:
